// zzfx-music.js - Объединенная библиотека ZzFX и ZzFXM
...
// ZzFXM - ZzFX Music - Version 1.1.8
/**
...
// Функция-помощник для преобразования массивов в строки для zzfxM
const zzfxMusicStringify = (music) => {
  const patterns = [];
  const instruments = [];

  music.forEach(channel => {
    instruments.push(channel[0].join(','));
    channel.slice(1).forEach(pattern => {
      patterns.push(pattern.join(','));
    });
  });

  return [instruments.join(';'), patterns.length, patterns];
};

export { zzfx, zzfxP, zzfxG, zzfxX, zzfxM, zzfxMusicStringify };